...
class ConfigStateController(engineDefaultState.DefaultState):
    ...
    def event_manager(self):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                exit()
            if self.button_velocity_increase1.check_if_clicked(event=event.type):
            ...
            if self.button_velocity_decrease1.check_if_clicked(event=event.type):
                self.trajectory1.decrease_initial_velocity()
                self.label_velocity_value1.set_text(self.trajectory1.get_initial_velocity())
                self.trajectory1.calculate()
                self.end_point1.set_default_position()
            if self.button_angle_increase1.check_if_clicked(event=event.type):
                self.trajectory1.increase_angle()
                self.label_angle_value1.set_text(self.trajectory1.get_angle())
                self.trajectory1.calculate()
                self.end_point1.set_default_position()
            if self.button_angle_decrease1.check_if_clicked(event=event.type):
                self.trajectory1.decrease_angle()
                self.label_angle_value1.set_text(self.trajectory1.get_angle())
                self.trajectory1.calculate()
                self.end_point1.set_default_position()
            if self.button_velocity_increase2.check_if_clicked(event=event.type):
            ...
            if self.button_velocity_decrease2.check_if_clicked(event=event.type):
                self.trajectory2.decrease_initial_velocity()
                self.label_velocity_value2.set_text(self.trajectory2.get_initial_velocity())
                self.trajectory2.calculate()
                self.end_point2.set_default_position()
            if self.button_angle_increase2.check_if_clicked(event=event.type):
                self.trajectory2.increase_angle()
                self.label_angle_value2.set_text(self.trajectory2.get_angle())
                self.trajectory2.calculate()
                self.end_point2.set_default_position()
            if self.button_angle_decrease2.check_if_clicked(event=event.type):
            ...
            if self.button_hide_hud.check_if_clicked(event=event.type):
                if "Hide" == self.button_hide_hud.get_text():
                    ...
                    if True == self.start_point2.is_added():
                        self.button_add_second_house.set_invisible()
                    self.button_hide_hud.set_text("Hide")

            if self.button_add_second_house.check_if_clicked(event=event.type):
                self.start_point2.add()
                self.button_add_second_house.set_invisible()
                self.button_velocity_increase2.set_enable()
                self.button_velocity_decrease2.set_enable()
                self.button_angle_increase2.set_enable()
                self.button_angle_decrease2.set_enable()
                self.button_remove_second_house.set_enable()
                self.button_add_second_house.set_disable()
            elif self.button_remove_second_house.check_if_clicked(event=event.type):
                self.start_point2.remove()
                self.button_add_second_house.set_visible()
                self.button_velocity_increase2.set_disable()
                self.button_velocity_decrease2.set_disable()
                self.button_angle_increase2.set_disable()
                self.button_angle_decrease2.set_disable()
                self.button_remove_second_house.set_disable()
                self.button_add_second_house.set_enable()
    ...
    def draw(self):
        ...
        if True == self.button_add_second_house.check_status():
    ...
    def create_gui(self):
        ...
        self.container = container.Container(   self.window, 
                                                pygame.math.Vector2(self.local_window_width/2-self.default_settings["hud_config"]["width"]/2, self.local_widow_height-self.default_settings["hud_config"]["height"]),
                                                pygame.math.Vector2(self.default_settings["hud_config"]["width"], self.default_settings["hud_config"]["height"]))
    ...
    def update_object_position(self, object):
        ...
        elif pygame.mouse.get_pos()[1] > self.local_widow_height-object.get_rect().height//2:
            object.set_position_y(new_y=self.local_widow_height-object.get_rect().height//2)
        else:
